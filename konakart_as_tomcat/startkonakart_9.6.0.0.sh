...
# chiabre: 
# -- -Xmx1400m -Xms400m (moved to image JAVA_OPTS)
# ++ -XX:+AlwaysPreTouch
# export CATALINA_OPTS="-Xmx1400m -Xms400m -server -Dsolr.solr.home=${CATALINA_HOME}/solr -Djava.awt.headless=true -Dactivemq.store.dir=${CATALINA_HOME}/mq -Dkk.log4j.debug=true -XX:-OmitStackTraceInFastThrow -DjvmRoute=`hostname` "
 export CATALINA_OPTS="${CATALINA_OPTS} -XX:+AlwaysPreTouch -server -Dsolr.solr.home=${CATALINA_HOME}/solr -Djava.awt.headless=true -Dactivemq.store.dir=${CATALINA_HOME}/mq -Dkk.log4j.debug=true -XX:-OmitStackTraceInFastThrow -DjvmRoute=`hostname` "

# chiabre
# JMX_EXPORTER OPTSONS
export CATALINA_OPTS="${CATALINA_OPTS} -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=9000 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -javaagent:${JMX_EXPORTER_INSTALL_DIR}/jmx_prometheus_javaagent.jar=9404:${JMX_EXPORTER_INSTALL_DIR}/config.yml"
...
# Heap Dumps
export CATALINA_OPTS="${CATALINA_OPTS} -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=KonaKartHeapDump.hprof "

# String Deduplication - requires the G1 gabage collector
# export CATALINA_OPTS="${CATALINA_OPTS} -XX:+UseStringDeduplication "

# Kill process if it runs out of memory
#export CATALINA_OPTS="${CATALINA_OPTS} -XX:OnOutOfMemoryError=\"/bin/sleep 30;/bin/kill %p; /bin/sleep 60; /bin/kill -9 %p\""
