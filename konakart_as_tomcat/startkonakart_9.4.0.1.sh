...
# ++ -XX:+AlwaysPreTouch
# ORIG export CATALINA_OPTS="-Xmx1400m -Xms400m -server -Dsolr.solr.home=${CATALINA_HOME}/solr -Djava.awt.headless=true -Dactivemq.store.dir=${CATALINA_HOME}/mq -Dkk.log4j.debug=true -XX:-OmitStackTraceInFastThrow -DjvmRoute=`hostname` "
...
export CATALINA_OPTS="${CATALINA_OPTS} -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=9000 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -javaagent:/opt/jmx-exporter/jmx_prometheus_javaagent.jar=9404:/opt/jmx-exporter/config.yml"
